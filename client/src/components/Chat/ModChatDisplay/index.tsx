...
export const ModChatDisplay: React.FC<{
    ...
    socket: Socket;
    allUsers: any;
    setSelectedUser1: React.Dispatch<React.SetStateAction<userType>> | null;
    setSelectedUser2: React.Dispatch<React.SetStateAction<userType>> | null;
}> = ({ threadProfile, messages, setMessages, uid, thread, socket, allUsers, setSelectedUser1, setSelectedUser2 }) => {
    ...
    const selecHandler = (e: any) => {
        setSelectedUser1!(e.target.value);
        setSelectedUserA(e.target.value);
        console.log(e.target.value)
    }
    const selecHandler2 = (e: any) => {
        setSelectedUser2!(e.target.value);
        setSelectedUserB(e.target.value)
        console.log("user 2 select value" + e.target.value)
    }

    const handleGetChat = () => {
        if (selectedUserA === selectedUserB) {
            alert("Please select two different users to fetch")
        }
        console.log("User 1" + selectedUserA + " User 2" + selectedUserB);
            ...
            <div className="justify-start ml-10 my-3">
                <span className="text-sm font-bold">
                    ...
                    <select
                        ...
                        {allUsers && allUsers!.map((user: userType) => <option key={user.id} value={user.id}>{user.name}</option>)}
                    </select>
                    user 2
                    <select
                        onChange={selecHandler2}
                        value={selectedUserB}
                    >
                        {allUsers && allUsers!.map((user: userType) => <option key={user.id} value={user.id}>{user.name}</option>)}
                    </select>
